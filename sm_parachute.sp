...
Handle g_fallspeed = INVALID_HANDLE;
Handle g_enabled = INVALID_HANDLE;
Handle g_linear = INVALID_HANDLE;
Handle g_msgtype = INVALID_HANDLE;
Handle g_cost = INVALID_HANDLE;
Handle g_payback = INVALID_HANDLE;
Handle g_welcome = INVALID_HANDLE;
Handle g_roundmsg = INVALID_HANDLE;
Handle g_version = INVALID_HANDLE;
Handle g_model = INVALID_HANDLE;
Handle g_decrease = INVALID_HANDLE;
Handle g_button = INVALID_HANDLE;
...
public CvarChange_Enabled(Handle: cvar, const String: oldvalue[], const String: intvalue[]) {
...
public CvarChange_Linear(Handle: cvar, const String: oldvalue[], const String: intvalue[]) {
	if (StringToInt(intvalue) == 0) {
		for (int client = 1; client <= g_maxplayers; client++) {
...
public CvarChange_Cost(Handle: cvar, const String: oldvalue[], const String: intvalue[]) {
	if (StringToInt(intvalue) == 0) {
		for (int client = 1; client <= g_maxplayers; client++) {
			if (IsClientInGame(client) && IsPlayerAlive(client)) {
...
public CvarChange_Button(Handle: cvar, const String: oldvalue[], const String: intvalue[]) {
	if (StringToInt(intvalue) == 1) {
		SetButton(1);
	}
	else if (StringToInt(intvalue) == 2) {
		SetButton(2);
	}
}

public CvarChange_Model(Handle: cvar, const String: oldvalue[], const String: intvalue[]) {
	if (StringToInt(intvalue) == 0) {
		for (int client = 1; client <= g_maxplayers; client++) {
			if (IsClientInGame(client) && IsPlayerAlive(client)) {
				CloseParachute(client);
