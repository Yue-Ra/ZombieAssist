...
void AttemptRTV(int client)
{
	...
	if (g_Voted[client])
	{
		ReplyToCommand(client, "[SM] %t", "Already Voted", g_Votes, g_VotesNeeded - GetTeamClientCount(CS_TEAM_SPECTATOR));
		return;
	}
	...
	if (g_Votes >= g_VotesNeeded - GetTeamClientCount(CS_TEAM_SPECTATOR))
	{
		StartRTV();
	}
}
